...
/* 
    ...
    JZ		080122	RevPro now coming through in RPC file, no longer forcing to NULL
    JZ		082322	Added logic for VF
    JZ		082522	Added fields: [Finance Vendor ID],[Finance Vendor Name],[Line: Subscription: Invoice Owner ID]
					Added new join to pull in the correct SF ID for the Finance Vendor and the name for validation
	JZ 		100222	Deprecated old product rate plan lookup
					Created new product rate plan lookup table with less fields per new direction
                    Updated view to comment out PRPL join lines no longer being used
	JZ		041323	Updated logic for SCES join as it had two Bill to and no Sold to
    JZ		050423	Update Logic for SCES/SCEB to align with logic that was created to fix bad IDs from these fields.
*/
...
SELECT
    ...
    ACCVF.`Account: CRM Account ID` AS '[Finance Vendor ID]',   
    ACCVF.`Account: Name` AS '[Finance Vendor Name]',
    ...
    RPC.`Rate Plan Charge: RevPro Line Number` AS 'RPC.Legacy_Revpro_Line_Number__c',
    ...
    CASE WHEN ((SUB.`Subscription: Invoice Owner ID` != ACC.`Account: ID`)  AND ACC.`Account: Payment Term` = 'VF') THEN 'Vendor Financed'
		WHEN RPC.`Rate Plan Charge: Billing Period` = 'Month' THEN 'Monthly'
    ...
    SUB.`Subscription: Invoice Owner ID` AS '[Line: Subscription: Invoice Owner ID]',
    ...
    SCEB.`Contact ID` AS '[Line: BillToContactId]',
    ...
    SCES.`Contact ID` AS '[Line: SoldToContactId]',
...
/* replacement to left outer join. IF PRPL.MatchRatePlanCharge = 1, then must also join on the Product Rate Plan Charge ID */
...
LEFT JOIN (SELECT DISTINCT `Contact ID`, `Account ID`, `Email`, `Email2`
			FROM `cache_SalesforceContactExtract_new`
            GROUP BY `Account ID`, `Email` ) SCEB
		ON ( ACC.`Account: CRM Account ID` = SCEB.`Account ID`
		AND ACC.`Bill To: Work Email` = SCEB.`EMAIL`)
    
LEFT JOIN (SELECT DISTINCT `Contact ID`, `Account ID`, `Email`, `Email2`
			FROM `cache_SalesforceContactExtract_new`
            GROUP BY `Account ID`, `Email` ) SCES
		ON ( ACC.`Account: CRM Account ID` = SCES.`Account ID`
		AND ACC.`Sold To: Work Email` = SCES.`EMAIL`) 
        
LEFT JOIN (SELECT `Account: CRM Account ID`,`Account: Name`, `Account: ID` FROM `cache_Zuora_Account`) ACCVF -- Get the SF ID for the vendor account
ON SUB.`Subscription: Invoice Owner ID` = ACCVF.`Account: ID`

WHERE (RPC.`Rate Plan Charge: Charge Type` = 'One-Time' 
	OR (RPC.`Rate Plan Charge: Charge Type` = 'Recurring' AND  RPC.`Rate Plan Charge: Effective End Date` >= date_sub(curdate(),interval 2 day)))
...
-- AND SUB.`Subscription: Name` = 'A-S00263156'
-- GROUP BY SUB.`Subscription: ID`   -- Uncomment ONLY to check how many contracts we expect to migrate
